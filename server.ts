...
let id:number=0;
const mensajes:Array<object>=JSON.parse(fs.readFileSync('public/chat.txt',"utf-8"));
...
router.get('/productos/listar/:id',(req,res)=>{
    ...
    let resultado:object={error: 'producto no encontrado'};
...
router.delete('/productos/borrar/:id',(req,res)=>{
    ...
    let arrayAux=[];
    let resultado:object={error: 'producto no eliminado: no se encontro'};
    for (let index = 0; index < productos.length; index++) {
        if(productos[index].id==params.id){
            resultado=productos[index];
        }else{
